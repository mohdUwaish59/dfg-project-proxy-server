...
async function createProxyLink(proxyId, realUrl, groupName, maxUses = 2000, treatmentTitle = null, postExperimentRedirectUrl = null, otreeSessionId = null) {
  const database = await connectToDatabase();
  return await database.collection('proxy_links').insertOne({
    ...
    max_uses: maxUses, // Total room capacity (default 2000)
...
async function recordLinkUsage(proxyId, sessionId, userIp, userFingerprint, participantNumber, prolificPid = null) {
  const database = await connectToDatabase();
  ...
  return await database.collection('link_usage').insertOne({
    ...
    joined_at: joinedAt,
    participant_timer_start: joinedAt, // Individual 15-minute timer start
...
// Automatically expire participants whose 15-minute timer has run out
async function expireTimedOutParticipants(proxyId) {
  const database = await connectToDatabase();
  const now = new Date();
  const tenMinutesAgo = new Date(now.getTime() - 900000); // 15 minutes = 900,000 ms
  
  // Find all waiting participants whose timer started more than 15 minutes ago
...
async function checkParticipantExpiration(proxyId, userFingerprint) {
  ...
  if (!participant || !participant.participant_timer_start) {
    return { expired: false, timeLeft: 900, timerStart: null };
  }
  ...
  const timeLeft = Math.max(0, 900 - elapsed); // 15 minutes = 900 seconds
...
async function deleteExpiredParticipant(proxyId, userFingerprint) {
  const database = await connectToDatabase();
  return await database.collection('link_usage').deleteOne({
    proxy_id: proxyId,
    user_fingerprint: userFingerprint
  });
}
